...
<iframe width="560" height="315" src="http://www.youtube.com/embed/tHEVRahx0WE?rel=0" frameborder="0" allowfullscreen></iframe>
...
<iframe width="560" height="315" src="http://www.youtube.com/embed/z9Xe3IfkNQU?rel=0" frameborder="0" allowfullscreen></iframe>
...
<iframe width="560" height="315" src="http://www.youtube.com/embed/Wx-Kr9cp7O8?rel=0" frameborder="0" allowfullscreen></iframe>
...
`starwars` is an example of a **dataframe**: a table of data with rows and columns. Each row contains an *observation*, and each columns holds a *variable*.
...
For example, 
* we should pass in a dataframe in the `ggplot` function: here we pass in the `starwars` dataframe
...
```{r adaeqqcacfofqcxs, exercise = TRUE}
ggplot(starwars) +
...
```{r non_blue_histogram, include = FALSE}
ggplot(starwars) + 
...
ggplot(starwars) +
  geom_histogram(mapping = aes(x = birth_year, fill = birth_year))
```

```{r fdeedqhioyiosqoh-solution}
ggplot(starwars) +
  geom_histogram(mapping = aes(x = birth_year, fill=birth_year), position = "identity")
```
...
ggplot(starwars) +
  geom_histogram(mapping = aes(x = birth_year, fill=birth_year), position="identity", alpha=0.3)
```
...
# check code
gradethis::grade_code(message = '"These aren`t the droids you`re looking for."')
```
...
ggplot(data = starwars) +
  geom_scatter(mapping = aes(x = height, y = mass))
```
...
<iframe width="560" height="315" src="http://www.youtube.com/embed/p6J9IxSBMwo?rel=0" frameborder="0" allowfullscreen></iframe>
